...
public class JSONMessageConverterRestController {
	...
	@GetMapping(value = "/news",produces = "application/json;chrset=UTF-8")
	public List<News> newsTitlesListJson(){
		return this.getNewsList();
	}
	@GetMapping(value = "/news/{no}",produces = "application/json;chrset=UTF-8")
	public News newsTitleJson(@PathVariable(name="no") int no){
		News news=getNewsList().get(no-1);
		return news;
	}
	
	@GetMapping(value = "/map/news",produces = "application/json;chrset=UTF-8")
	...
	@GetMapping(value="/map/news/{no}",produces = "application/json;chrset=UTF-8")

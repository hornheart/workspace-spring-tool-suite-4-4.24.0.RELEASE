...
public class AuthLoginAnnotationInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		 ...
		 */
		/***********************************************************************************
		 * 1. handler객체 종류 확인 우리가 관심 있는 것은 @Controller객체에 있는 매핑된(@RequestMapping이 붙은)
		 * 메서드이므로 HandlerMethod 타입인지 체크
		 *************************************************************************************/
		...
		/***************************
		 * 4. HandlerMethod객체에 @LoginCheck어노테이션 이없는 경우, 즉 인증이 필요 없는 요청
		 ***************************/
		if (loginCheck == null) {
		...
		/***************************
		 * 4. HandlerMethod객체에 @LoginCheck어노테이션 이있는 경우, 세션이 있는지 체크
		 ***************************/
		...
		if (sUserId == null) {
			...
			if (!requestURI.startsWith("/user/logout")) {
				if ((requestURI.startsWith("/user/login") && request.getMethod().equalsIgnoreCase("GET"))) {
					System.out.println("--->");
					return true;
				}
				/*

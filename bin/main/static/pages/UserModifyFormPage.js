...
export const UserModifyFormPage = async (userId) => {
  ...
  const userModifyAction=async()=>{
		...
		const responseJsonObject =await userApi.userModifyAction(sendJsonObject);
		console.log(responseJsonObject);
		(responseJsonObject.status==ResponseStatusCode.UPDATE_USER)?location.hash=`#/user_view/${responseJsonObject.data.userId}` : ''; 
		...
		<tr>
			<td>
				...
				<table width="590" border="0" cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							<td align="center"><input id="btn_user_modify_action" type="button" value="수정"> &nbsp;</td>
						</tr>
					</tbody>
				</table>
